...
const DeleteConfirmDialog = ({
  ...
  <div className={styles.sheetBackdrop} role="presentation" onClick={onCancel}>
    <section
        ...
        <div className={styles.confirmGroupPlayers}>
          {group.players.map((player, index) => (
            <RoughAvatar
              ...
              color={player.color}
              seed={seedFromText(
                `confirm-${group.id}-${index}-${player.name}-avatar`,
              )}
              size="small"
            />
          ))}
...
const renderHelpDialog = (onClose: () => void) => (
  <div className={styles.sheetBackdrop} role="presentation" onClick={onClose}>
    ...
    >
      <header className={styles.helpHeader}>
        <h3 id="help-title">{t("onboardingTitle")}</h3>
        <button
      ...
      <ol className={styles.onboardingList}>
        <li>{t("onboardingAddPlayers")}</li>
        <li>{t("onboardingCreateSheet")}</li>
        <li>{t("onboardingAdjustScores")}</li>
      </ol>
    </section>
  </div>
);
...
const GroupCard = observer(
  ({
    ...
    <div className={styles.groupCard}>
      ...
      <button className={styles.groupPlayArea} type="button" onClick={onPlay}>
...
export const NewSheet = observer(({ store }: { store: RootStoreInstance }) => {
  ...
  const [groupToDelete, setGroupToDelete] = useState<SavedGroupInstance | null>(
    null,
  );
  ...
  const recentSheet =
    store.sheets.length > 0
      ...
      : null;
  const showOnboarding = store.sheets.length === 0 && recentGroups.length === 0;

  useEffect(() => {
    if (showAllGroups || showSettings || showHelp || groupToDelete) {
  ...
  }, [showAllGroups, showSettings, showHelp, groupToDelete]);

  const handleSubmit = (event: FormEvent) => {
    event.preventDefault();

    const playerNames = splitNames(namesText);
    if (playerNames.length === 0) {
      setPlayersError(t("playersRequired"));
      return;
    }

    setPlayersError("");
    store.createSheet(playerNames);
  };
  ...
  return (
    <section className={styles.screen}>
          ...
          <label className={styles.label}>
            ...
            <textarea
              ...
              onChange={(event) => {
                ...
                setNamesText(nextValue);
                if (playersError && splitNames(nextValue).length > 0) {
                  setPlayersError("");
                }
              }}
              ...
              aria-describedby={playersError ? "players-error" : undefined}
            />
            {playersError ? (
              <p className={styles.formError} id="players-error" role="alert">
                {playersError}
              </p>
            ) : null}
          </label>
      ...
      {showSettings ? (
        <SettingsDialog store={store} onClose={() => setShowSettings(false)} />
      ) : null}

      {showHelp ? renderHelpDialog(() => setShowHelp(false)) : null}
